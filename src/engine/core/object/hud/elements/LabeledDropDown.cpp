...
namespace Gengine {
        ...
        void LabeledDropDown::init(std::string text,
            ...
            std::cout << "Done creating buttons" << std::endl;

            std::cout << "Current label Position: " << _position.x << ", " << _position.y << std::endl;
            _label.init();
        ...
        void LabeledDropDown::draw(sf::RenderTarget* target) {
            // sf::RectangleShape bg = sf::RectangleShape(sf::Vector2f(_size.x, _size.y));
            // bg.setPosition(_position);
            // bg.setFillColor(sf::Color::Black);
            // target->draw(bg);

            _label.draw(target);
            _currentOption.draw(target);

            if (_isOpen) {
                for (int i = 0; i < _options.size(); i++) {
                    _options[i]->draw(target);
                }
            }
        }
